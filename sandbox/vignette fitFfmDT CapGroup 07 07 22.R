...
commonNames <- intersect(names(stocksCRSP),names(factorsSPGMI))
facModDatmerged <- merge(stocksCRSP, factorsSPGMI, by = commonNames)
names(facModDatmerged)

colNames <- c("Date","TickerLast","CapGroupLast","Return","Ret13WkBill",         
              "mktIndexCRSP","BP","LogMktCap","SEV")  
facDatLong <- facModDatmerged[, .SD , .SDcols = colNames]
# change the names of some of the columns
setnames(facDatLong, old = c("Ret13WkBill" , "mktIndexCRSP" ,"LogMktCap"  ),
         new = c("RF","MKT","SIZE"))
length(unique(facDatLong$TickerLast)) # 300 stocks
facDat <-facDatLong[facDatLong$Date >= as.Date("2006-01-31") & 
                          facDatLong$Date <= as.Date("2010-12-31"), ]

# Create an xts object returns for 20 stocks
returnMat = tapply(facDat[["Return"]],list(facDat$Date,facDat$TickerLast),I)
...
facDat20 <- facDat[TickerLast %in% Tickers20,]
...
wtsGmvLO <- out.lo$weights

names <- c("WTS","MU","SD","SR")
out.lo <- opt.outputMvo(opt.lo,returns,names=names,digits = 4)
wtsGmvLO <- opt.lo$WTS
length(wtsGmvLO)

nms <- unique(facDat$TickerLast)

wtsGmvLO <- rep(1/300,300)
names(wtsGmvLO) <- nms
...
ret.var="Return" 
date.var = "Date"
exposure.vars= c("CapGroupLast","SIZE","BP","SEV")
spec1 <- specFfm(data = facDat,asset.var = asset.var, ret.var = ret.var, 
...
# Something is weird about the adjusted R-squared values
...
exposure.vars= c("CapGroupLast","SIZE","BP")
...
spec1 <- specFfm(data = facDat, asset.var="TickerLast", ret.var="Return",
...
# The above line results in a Warning message
